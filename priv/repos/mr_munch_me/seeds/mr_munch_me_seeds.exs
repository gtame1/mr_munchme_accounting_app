...
# Reuse SeedHelper from core_seeds.exs (already loaded)

defmodule MrMunchMeSeedHelper do
...
accounts = [
  ...
  %{code: "1005", name: "Stripe Receivable", type: "asset", normal_balance: "debit"},
...
products
|> Enum.each(&MrMunchMeSeedHelper.upsert_product/1)
...
ingredients
|> Enum.each(&MrMunchMeSeedHelper.upsert_ingredient/1)
...
locations
|> Enum.each(&MrMunchMeSeedHelper.upsert_location/1)
...
recipe_count =
  ...
  |> Enum.map(fn sku ->
    ...
    MrMunchMeSeedHelper.upsert_recipe(sku, initial_recipe_date, recipe_lines)
    length(recipe_lines)
  end)
  |> Enum.sum()
